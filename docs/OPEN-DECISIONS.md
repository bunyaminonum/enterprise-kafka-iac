...
| OD-04 | Source of TLS certificates, Kerberos keytabs and Secret Protection files (Vault, CyberArk, ...) and how pipelines fetch them | cp-ansible reads keytabs and `security.properties` on the control node; the pipelines contain a placeholder step | `shared/base/10-security.yml`, `.github/workflows/deploy.yml`, `bitbucket-pipelines.yml` |
